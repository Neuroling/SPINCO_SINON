function [s,f] = ltas_TH(x,fs,varargin)
...
%   Copyright 2016 University of Surrey.
    ...
    if options.graph
        pl = semilogx(f,rearrange(s,order,[dims_out_shift(1) prod(dims_out_shift(2:end))]));
        if options.xL
            xlabel('Frequency (Hz)');
        end
        if options.xTickL
            set(gca,'XTick',options.xTickL);
        end
        if options.yL
            ylabel(labelY);
        end
